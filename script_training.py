...
print('bs=%d, lr=%f, entrloss=%f, shr=%f, memdim=%d' %
      (batch_size_in, learning_rate, entropy_loss_weight,
       sparse_shrink_thres, mem_dim_in))
...
if(opt.ModelName == 'MemAE'):
    model = AutoEncoderCov3DMem(chnum_in_, mem_dim_in,
...
# -------------------------------------------------
resume_epoch = 0
checkpoint_files = [f for f in os.listdir(saving_model_path)
                    if f.endswith(".pt")]
if checkpoint_files:
    latest_ckpt = sorted(checkpoint_files)[-1]
    ckpt_path = os.path.join(saving_model_path, latest_ckpt)
    print(f"Resuming training from checkpoint: {ckpt_path}")
    model.load_state_dict(torch.load(ckpt_path))
    if "epoch_" in latest_ckpt:
        try:
            resume_epoch = int(latest_ckpt.split("epoch_")[1]
                               .split(".pt")[0])
        except:
            resume_epoch = 0
...
# Training loop
# -------------------------------------------------
for epoch_idx in range(resume_epoch, max_epoch_num):
    for batch_idx, (item, frames) in enumerate(tr_data_loader):
        ...
        if(opt.IsTbLog):
            ...
            if((global_ite_idx % tb_img_log_interval) == 0):
                ...
                frames_recon_vis = np.concatenate(frames_recon_vis,
                                                  axis=-1)
        ...
        if((batch_idx % textlog_interval) == 0):
            print('[%s, epoch %d/%d, bt %d/%d] loss=%f, rc_losss=%f, ent_loss=%f'
                  % (model_setting, epoch_idx, max_epoch_num,
                     batch_idx, data_loader_len,
    ...
    if((epoch_idx % save_check_interval) == 0):
        torch.save(model.state_dict(),
                   '%s/%s_epoch_%04d.pt' % (saving_model_path,
                                            model_setting, epoch_idx))

torch.save(model.state_dict(),
           '%s/%s_epoch_%04d_final.pt' % (saving_model_path,
                                          model_setting, epoch_idx))
